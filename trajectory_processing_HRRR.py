...
npp_folder = os.path.join(
    '/rcfs/scratch/corn062/CLMS/bio-geophysical/net-gross_primary_production/npp_global_300m_10daily_v2')
...
fapar_folder = os.path.join(
    '/rcfs/scratch/corn062/CLMS/bio-geophysical/vegetation_properties/fapar_global_300m_10daily_v2')
fnames = np.zeros(0, dtype=str)
...
fc_folder = os.path.join(
    '/rcfs/scratch/corn062/CLMS/bio-geophysical/vegetation_properties/fcover_global_300m_10daily_v2')
fnames = np.zeros(0, dtype=str)
...
## because each we know how many timepoints there are (because we know how many files we have) then we can
## pre-allocate
part_lon = np.ones((numpart, numtimes)) * -9999
part_lat = np.ones((numpart, numtimes)) * -9999
part_x = np.ones((numpart, numtimes)) * -9999
part_y = np.ones((numpart, numtimes)) * -9999
part_mass = np.ones((numpart, numtimes)) * -9999
part_hmix = np.ones((numpart, numtimes)) * -9999
part_z = np.ones((numpart, numtimes)) * -9999
part_ba = np.ones((numpart, numtimes)) * -9999
part_npp = np.ones((numpart, numtimes)) * -9999
part_ndvi = np.ones((numpart, numtimes)) * -9999
part_fapar = np.ones((numpart, numtimes)) * -9999
part_lai = np.ones((numpart, numtimes)) * -9999
part_fcover = np.ones((numpart, numtimes)) * -9999
...
for iidx, i in enumerate(fnamelist[0:2]):
    ...
    fname = os.path.join(folder, fnamelist[iidx])
    partout, time, numpart = fp.read_partposition_flexpart(fname)
    part_x_ = partout[:, 2]
    part_y_ = partout[:, 3]
    part_z[:, iidx] = partout[:, 4]
    part_hmix[:, iidx] = partout[:, 5]
    part_mass[:, iidx] = partout[:, 12]
    date = time
    year = str(date.year)
    ...
    part_lon_, part_lat_ = lcc_proj(part_x_, part_y_, inverse=True)
    part_x[:, iidx] = part_x_
    part_y[:, iidx] = part_y_
    part_lon[:, iidx] = part_lon_
    part_lat[:, iidx] = part_lat_
    ...
    ### 3. convert lon/lat to x/y
    ### 4. finds indices that correspond from particle x/y to VOI
    ...
    lon = lon[idx1]
    lat = lat[idx2]
    FCOVER = ds.variables['FCOVER'][0, idx2, idx1] * ds.variables['FCOVER'].scale_factor + ds.variables[
        'FCOVER'].add_offset
    ### get indices for each particle that corresponds to lon/lat variables
    lon_idx = np.digitize(part_lon_, lon)
    lat_idx = np.digitize(part_lat_, lat)
...
sname = os.path.join(folder, 'CLMS_processed_HRRR_%s.nc' % tmp.split('_')[1])
print(sname)
...
hmix_var = ds_out.createVariable(varname='hmix', datatype='f4', dimensions=('particle', 'time'))
